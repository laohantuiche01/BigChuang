...
class TurningFunction:
    ...
    @staticmethod
    def d2_perp_distance(tf_reduced_q: Tuple[np.ndarray, np.ndarray],
                         tf_reduced_c: Tuple[np.ndarray, np.ndarray]) -> float:
        """计算D₂^⊥距离（旋转不变，无近似损失）"""
        return TurningFunction.l2_distance(tf_reduced_q, tf_reduced_c)

    @staticmethod
    def d1_perp_distance(tf_reduced_q: Tuple[np.ndarray, np.ndarray],
                         tf_reduced_c: Tuple[np.ndarray, np.ndarray]) -> float:
        """计算D₁^⊥距离（旋转不变，2-近似）"""
        return TurningFunction.l1_distance(tf_reduced_q, tf_reduced_c)
...
class LSHFamily:
    ...
    def step_shift_lsh(self, tf: Tuple[np.ndarray, np.ndarray], num_shifts: int = None) -> List[str]:
        """step-shift-LSH（论文3.2节，支持任意c>1，无近似损失）"""
        f_x, f_y = tf
        if num_shifts is None:
            num_shifts = len(f_x) - 1  # 每个断点对应一次垂直移位
        hash_list = []
        for s in range(num_shifts):
            # 垂直移位：将第s个断点的y值设为0
            shift_val = f_y[s]
            f_y_shifted = f_y - shift_val
            # 生成random-point-LSH哈希
            hash_bits = []
            for _ in range(100):
                x = np.random.uniform(0, 1)
                y = np.random.uniform(0, self.span_m)
                idx = np.searchsorted(f_x, x) - 1
                f_val = f_y_shifted[idx] if idx >= 0 else f_y_shifted[0]
                if f_val > y + 1e-8:
                    hash_bits.append('1')
                elif f_val < y - 1e-8:
                    hash_bits.append('-1')
                    ...
                    hash_bits.append('0')
            hash_list.append(''.join(hash_bits))
        return hash_list
